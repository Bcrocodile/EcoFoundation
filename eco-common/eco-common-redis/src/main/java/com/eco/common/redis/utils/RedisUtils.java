...
public class RedisUtils {
     ...
     * 默认缓存3600秒
     */
    private final static Long DEFAULT_TIMEOUT = 3600L;
    /**
    ...
    public <T> T get(String key, Supplier<T> fun) {
        return get(key, fun, DEFAULT_TIMEOUT);
    }
     ...
     * @return
     */
    public <T> T get(String key, Supplier<T> fun, long time) {
        if (null == key) {
            return null;
        }
    ...
    public Set<Object> reverseRange(String key ,int start,int end) {
        try {
            Set<Object> keys = redisTemplate.opsForZSet().reverseRange(key,start,end);
            return keys;
        } catch (Exception e) {
            e.printStackTrace();
            return null;
     ...
     * @return
     */
    public Long rszset(String key, Set<ZSetOperations.TypedTuple<Object>> pars, long time) {
        try {
            Long keys = redisTemplate.opsForZSet().add(key, pars);
            if (time > 0) {
                expire(key, time);
            }
            return keys;
     ...
     * @param delta 负数减分
     */
    public void incrementScore(String key,Long id,double delta) {
        redisTemplate.opsForZSet().incrementScore(key, id, delta);
    }

//    public void incrementScore(String key,Long id,double delta) {
//        redisTemplate.opsForZSet().(key, id, delta);
//    }
}
